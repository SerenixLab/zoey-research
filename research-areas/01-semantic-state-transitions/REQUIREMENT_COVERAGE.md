...
Status: `implementation conclusions suspended pending corrective revalidation`
...
listed, but all implemented-frontier satisfaction claims are suspended pending
fresh review of corrective workbench baseline `6cc623b`.
...
| --- | --- | --- | --- | --- |
| `SST-R01` | SCM and selected-slice ADRs define compositional roles and lifecycle meaning. | Raw communication, attribution, scoped correction/control, direct and later dispositions, realizations, candidate, activation assessment, active trial, later applicability, projection, outcome, uncertainty, explanation, support, and limitations remain distinct; corrective baseline `6cc623b` hardens exact family and creator checks. | Successor lifecycle actions and general conflict remain deferred; fresh corrective review is pending. | `implementation evidence present; acceptance suspended`; no general statechart need demonstrated. |
...
| `SST-R05` | ADR-006/007 require exact material basis, current eligibility, and fail-closed invalidity. | Corrective baseline `6cc623b` covers earlier equal-payload evidence, wrong identity, stale basis, projection corruption, strict creator/result multiplicity, ingestion closure, processing rollback, output ambiguity, and changed redelivery without repair. | Cross-process database concurrency/CAS is not required by this run-scoped path; fresh corrective review is pending. | `implementation evidence present; acceptance suspended`. |
| `SST-R06` | Source, basis, support, transition ancestry, actor, and status origin are distinct. | Source, ingestion basis, epistemic support, transition ancestry, realization, and typed projection relations remain separately implemented and were hardened symmetrically at `6cc623b`. | Formal behavior-configuration provenance remains gated by `EVAL-004`; fresh corrective review is pending. | `implementation evidence present; acceptance suspended`. |
| `SST-R07` | Fixture/inference evidence cannot silently become authority or durable state. | SUT owns interpretation. Corrective baseline `6cc623b` retains bounded applicability, uncertainty, and an explanation oracle that rejects mixed global, permanent, preference, style, causal, efficacy, fatigue, focused-drill, and hidden-reasoning promotion. | Formal evidence authority remains separately gated; fresh corrective review is pending. | `implementation evidence present; acceptance suspended`. |
...
| `SST-R10` | Silent last-write-wins and cyclic self-validation are forbidden. | Corrective baseline `6cc623b` rejects competing state and terminal families, rolls back failed semantic processing, and raises transparent integrity failure for multiple outputs without harness arbitration. | General retained conflict objects, concurrent successor transitions, cyclic non-convergence, and fresh corrective review remain deferred. | `implementation evidence present; acceptance suspended`; no framework gap demonstrated. |
| `SST-R11` | ADR-005/008 and the profile define closed ingress, public-boundary evidence, simulator isolation, and passive inspection. | Fixture projection strips evaluation meaning; SUT owns behavior selection; simulator reports fidelity; corrected checkpoint/capture attacks remain passive; dependency gates reject SUT-to-evaluation paths. | Future fixture/simulator roles and fresh corrective review require the same protections. | `implementation evidence present; acceptance suspended`. |
...
- The Area 1 mechanism investigation remains closed, but implemented-frontier
  acceptance is suspended pending fresh independent review of exact corrective
  range `4376bcc..6cc623b`. Historical acceptance of earlier exact corrective
  commits remains recorded but does not attest the broader correction.
