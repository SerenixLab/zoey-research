...
The active SCN-001 workbench has implemented the selected-slice chain through exact direct current-session correction realization and non-active delayed-correction candidate formation. Delayed activation, later applicability and outcomes, explanations, formal evaluation, and scoring remain unimplemented.
...
| --- | --- | --- | --- | --- | --- |
| **1. Semantic state and transitions** | Preserve assertions, observations, hypotheses, candidates, active trials, outcomes, corrections, narrowing, and retirement as distinct states. | SCM; `ADR-003`, `ADR-006` | Selected-slice run state and transition evidence is implemented through exact direct current-session correction realization and separate, non-active delayed-correction candidate formation. | **Initially reviewed:** Iris implements fail-closed memory-candidate transitions, source binding, and review promotion; Yuki implements current playbook loading/inspection but not correction history. | The delayed-candidate trigger in [`01-semantic-state-transitions`](research-areas/01-semantic-state-transitions/RESEARCH_BRIEF.md) was re-triaged after implementation without exposing a concrete mechanism gap. Activation is the next engineering frontier; broader research remains dormant. XTDB is `source-reviewed` as `EXT-STATE-001`; no mechanism or experiment is adopted. |
