...
| --- | --- | --- | --- | --- | --- | --- |
| `EXT-STATE-001` | `1` Semantic state and transitions | [`XTDB`](https://github.com/xtdb/xtdb) | `architecture-reference`, `pattern-extraction-source`, conditional `mechanism-candidate` | `source-reviewed` | Bounded temporal-projection source review complete. Area 1 closed at the current V-003 frontier without activating the contingent experiment; retain XTDB as a trigger-bound reference. No storage choice, dependency, clone, compatibility claim, or workbench is active. | [`external-capabilities/xtdb/`](external-capabilities/xtdb/ASSESSMENT.md) |
